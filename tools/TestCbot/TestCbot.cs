// FULL REAL CBOT — copied from TradingEngineCBot.cs for bisection test
using System;
...
public sealed class TestCbot : Robot
{
    ...
    // ── Exact copy of TradingEngineCBot, with CBOT| prefix for tracing ──
                        ...
                        if (commands.ValueKind == JsonValueKind.Array)
                        {
                            foreach (var cmd in commands.EnumerateArray())
                            {
                                ...
                                if (cmdType == "submit_order")
                                    execs.Add(ExecuteSubmitOrder(cmd));
                                else if (cmdType == "close_position")
                                    execs.Add(ExecuteClosePosition(cmd));
                                else if (cmdType == "close_partial")
                                    execs.Add(ExecuteClosePartialPosition(cmd));
                                else if (cmdType == "modify_order")
                                    execs.Add(ExecuteModifyOrder(cmd));
        ...
        foreach (var pos in Positions)
        {
            if (_positionMap.TryGetValue(pos.Id, out var orderGuid) && orderGuid.ToString() == positionIdStr)
            {
                ...
                return MakeExecResult(clientOrderId.ToString(), "close", pos.Id,
                    ...
                    pos.CurrentPrice > 0 ? pos.CurrentPrice : 0d,
                    pos.VolumeInUnits / (Symbols.GetSymbol(pos.SymbolName)?.LotSize ?? 100000.0), null,
