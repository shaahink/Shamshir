"""iter-viability V4: session/time-of-day pure-OOS census driver (Lane R, cloned from census_driver.py).

Runs the 80-cell V4 census (4 session strategies x 10 FX symbols x {M15,H1}, D13 one cell per
...
V4 experiment. Pre-registered in LEDGER.md Session 7. Same run config as V2 except the strategy
set (net-new session family) and timeframes (M15 native + H1 cross-check). maxDdEnabled=False
(F82 research mode), raw per-bar dukascopy spread, --parallel 3 (S5 engine cap), F83 idempotency
namespacing by exp_id[:8] kept verbatim.

Prerequisite: the 4 session configs must be in trading.db. Starting TradingEngine.Web with the
new config/strategies/*.json present auto-upserts them via ConfigSyncService.SyncAsync (a JSON
file with no DB row classifies as Resynced -> upsert). No manual seeding needed.

Usage:
  python tools/research/v4_census_driver.py --create-experiment          # prints new experiment id
  python tools/research/v4_census_driver.py --experiment <guid> [--pilot] [--parallel 3] \
...
STRATEGIES = ["london-orb", "ny-open-drive", "asia-range", "day-of-week"]
# FX only (pre-reg): 7 FX-majors + 3 JPY-crosses. Metals/crypto excluded — session/time-of-day is
# an FX-session phenomenon and XAG/XAU/BTC/ETH carry the F77 1-pip-TypicalSpread cost artifact.
SYMBOLS = ["AUDUSD", "EURGBP", "EURUSD", "GBPUSD", "NZDUSD", "USDCAD", "USDCHF",
           "EURJPY", "GBPJPY", "USDJPY"]
TIMEFRAMES = ["M15", "H1"]

# Pilot cells (pre-reg): two cells that CAN fail the hypothesis and should trade often on M15, so
# the trade count guards against the F78/F79 governor/daily-DD starvation the session family
# exposed in the V2 pilot. A liquid FX major on native M15 for an ORB and a drive strategy.
#   london-orb/EURUSD/M15   — opening-range breakout, fires most sessions
#   ny-open-drive/GBPUSD/M15 — one directional entry per NY open
PILOT = [("london-orb", "EURUSD", "M15"), ("ny-open-drive", "GBPUSD", "M15")]

EXPERIMENT_NAME = "v4-session-tod"
HYPOTHESIS = ("V4 session/time-of-day pure OOS census 2019-2023 (D13, one cell per run); "
              "pre-registered in iter-viability LEDGER.md Session 7. H-SESSION family-pooled "
              "$/position > 0 (block-bootstrap CI excludes 0); per-strategy + H-TF (M15 vs H1); "
              "spread policy = raw per-bar dukascopy + 1.5x/2x post-hoc stress. Stop rule binding.")
# Same scoring weights as V2 (075D5240 / v2-frozen-bank-oos) so sv2 composites stay comparable (D4).
SPEC = {"Name": EXPERIMENT_NAME, "Hypothesis": HYPOTHESIS,
        "Symbols": SYMBOLS, "Timeframes": TIMEFRAMES, "Strategies": STRATEGIES,
        "From": "2019-01-01", "To": "2023-12-31", "WalkForward": None, "MaxRuns": 80,
        "Variants": [],
...
def run_body(strategy, sym, tf, exp_id):
    return {
        ...
        "idempotencyKey": f"v4-census-{exp_id[:8]}-{strategy}-{sym}-{tf}".lower(),
    }
