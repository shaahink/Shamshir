...
public sealed class KernelBacktestLoop
{
    ...
    private long _timingBars;
    private long _timingEvaluateMs;
    private long _timingPumpMs;
    private long _timingCompleteBarMs;
    private long _timingJournalSteps;
    ...
    public TimingReport? TimingReport { get; private set; }
    ...
    public async Task<EngineState> RunAsync(IEventTape tape, EngineState initial, CancellationToken ct)
    {
        ...
        {
            if (_diagnostics && _timingBars > 0)
                TimingReport = new TimingReport(_timingBars, _timingEvaluateMs, _timingPumpMs, _timingCompleteBarMs, _timingJournalSteps);
        }
    ...
    public async Task<EngineState> RunFromBrokerAsync(EngineState initial, CancellationToken ct)
    {
        ...
        finally
        {
            ...
            {
                TimingReport = new TimingReport(_timingBars, _timingEvaluateMs, _timingPumpMs, _timingCompleteBarMs, _timingJournalSteps);
                _logger.LogWarning("KERNEL|TIMING|bars={Bars}|evaluate(ms)={EvalMs}|pump(ms)={PumpMs}|completeBar(ms)={CbarMs}|journalSteps={Steps}|total(ms)={TotalMs}",
                    _timingBars, _timingEvaluateMs, _timingPumpMs, _timingCompleteBarMs, _timingJournalSteps,
                    _timingEvaluateMs + _timingPumpMs + _timingCompleteBarMs);
            }
        }
    ...
    private async Task<EngineState> ProcessBarAsync(BarClosed bar, EngineState state, CancellationToken ct)
    {
        ...
        var pumpSw = _diagnostics ? Stopwatch.StartNew() : null;
        state = await PumpAsync(state, ct);
        ...
        if (state.ExitMode == ExitMode.VenueManaged)
        {
            ...
            if (rec.Effects.Count > 0)
            {
                ...
                var step = new StepRecord(
                    ...
                    EventKind: "Reconcile",
                    EventJson: "",  // F3: serialized in background sink
                    EffectKinds: rec.Effects.Select(e => e.GetType().Name).ToArray(),
                    EffectsJson: "",  // F3: serialized in background sink
                    Risk: _captureRisk(state),
                    Regime: null,
                    DecisionReason: "RECONCILED",
                    StrategyVerdicts: Array.Empty<StrategyVerdict>())
                {
                    RawEvent = new { venueOpenIds = venueOpenIds.Count },
                    RawEffects = rec.Effects,
                };
        ...
        var evalSw = _diagnostics ? Stopwatch.StartNew() : null;
        var eval = await _evaluator.EvaluateAsync(bar, state, ct);
        if (evalSw is not null) { _timingEvaluateMs += evalSw.ElapsedMilliseconds; evalSw.Stop(); }
        ...
        var cbarSw = _diagnostics ? Stopwatch.StartNew() : null;
        await _venue.CompleteBarAsync(ct);
        if (cbarSw is not null) { _timingCompleteBarMs += cbarSw.ElapsedMilliseconds; cbarSw.Stop(); }

        if (_diagnostics) _timingBars++;
    ...
    private async Task<EngineState> PumpAsync(EngineState state, CancellationToken ct)
    {
        ...
        while (progressed)
        {
            ...
            while (_queue.TryDequeue(out var evt))
            {
                ...
                if (_diagnostics) _timingJournalSteps++;
