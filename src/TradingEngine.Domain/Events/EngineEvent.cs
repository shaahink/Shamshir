...
public record OrderProposed(Guid OrderId, Symbol Symbol, TradeDirection Direction, OrderType OrderType, Price? LimitPrice, Price StopLoss, Price? TakeProfit, string StrategyId, decimal SignalPriceMid, decimal SlPips, decimal PipValuePerLot, DateTime OccurredAtUtc, ExternalVerdicts External = default, RiskProfile? Profile = null, string? EntryReason = null, string? EntryRegime = null, Timeframe EntryTimeframe = Timeframe.H1, OrderEntryOptions? Entry = null) : EngineEvent(OccurredAtUtc);

public record OrderSubmitted(Guid OrderId, Symbol Symbol, TradeDirection Direction, decimal Lots, Price? LimitPrice, string StrategyId, DateTime OccurredAtUtc, OrderType OrderType = OrderType.Market, Price StopLoss = default, Price? TakeProfit = null, string? EntryReason = null, string? EntryRegime = null) : EngineEvent(OccurredAtUtc);
