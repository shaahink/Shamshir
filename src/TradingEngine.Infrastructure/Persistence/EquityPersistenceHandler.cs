...
public sealed class EquityPersistenceHandler : IEventHandler<EquityUpdated>, IAsyncDisposable
{
    ...
    public EquityPersistenceHandler(PersistenceService persistence, ILogger<EquityPersistenceHandler> logger, IRunDataCache? runDataCache = null)
    {
        ...
        while (!ct.IsCancellationRequested)
        {
            try
            {
                ...
                foreach (var group in buffer.GroupBy(b => b.RunId))
                {
                    ...
                    _cache?.AppendEquity(group.Key, snapshots);
                }
                buffer.Clear();
            }
