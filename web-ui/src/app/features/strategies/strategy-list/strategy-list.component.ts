...
@Component({
  ...
  template: `
    <div class="space-y-6">
        ...
        @for (s of strategies(); track s.id) {
          <div class="rounded-lg border border-gray-800 bg-gray-900/50 p-4 transition hover:border-gray-600">
            <div class="flex items-center justify-between mb-2">
              <a [routerLink]="['/strategies', s.id]" class="text-sm font-medium text-gray-100 hover:text-emerald-400">{{ s.displayName }}</a>
      ...
      @if (strategies().length === 0) { <div class="py-12 text-center text-sm text-gray-500">No strategies loaded.</div> }
    </div>
  `,
})
