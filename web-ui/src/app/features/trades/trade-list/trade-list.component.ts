  ...
  template: `
    <div class="space-y-6">
      ...
      <div class="overflow-x-auto rounded-lg border border-gray-800">
        ...
        @if (allTrades().length === 0) { <div class="px-4 py-8 text-center text-sm text-gray-500">No trades</div> }
      </div>
    </div>
  `,
})
export class TradeListComponent implements OnInit {
  ...
  fmtVal(row: TradeSummary, col: { key: string; label: string; format?: string; colorFn?: (v: number) => string }): string {
    ...
    const n = Number(v);
    switch (col.format) {
      case 'currency': return Number.isNaN(n) ? String(v) : n.toFixed(2);
      case 'pips': return Number.isNaN(n) ? String(v) : n.toFixed(1);
      case 'number': return Number.isNaN(n) ? String(v) : n.toString();
  ...
  async load(): Promise<void> {
    ...
    this.allTrades.set(data); this.page.set(1);
  }
  clear(): void { this.filterSymbol = ''; this.filterStrategy = ''; this.filterDirection = ''; this.filterFrom = ''; this.filterTo = ''; }
