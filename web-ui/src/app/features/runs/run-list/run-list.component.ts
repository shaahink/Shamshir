    ...
    <div class="space-y-6">
      <div class="flex items-center justify-between">
        ...
        <div class="flex gap-2">
          @if (selectedRuns().length >= 2) {
            <button (click)="compareOpen.set(!compareOpen())" class="rounded-md border border-emerald-700 px-3 py-1.5 text-xs text-emerald-400 hover:bg-emerald-900/20">Compare ({{ selectedRuns().length }})</button>
          }
          <a routerLink="/runs/new" class="rounded-md bg-emerald-600 px-4 py-2 text-sm font-medium text-white hover:bg-emerald-500">+ New Backtest</a>
        </div>
      </div>
      ...
      @if (compareOpen() && selectedRuns().length >= 2) {
        <div class="rounded-lg border border-emerald-800 bg-gray-900/50 p-4">
          ...
          <div class="overflow-x-auto"><table class="min-w-full text-xs"><thead class="text-gray-500"><tr><th class="px-3 py-1 text-left">Run</th><th class="px-3 py-1 text-right">Net P/L</th><th class="px-3 py-1 text-right">Max DD%</th><th class="px-3 py-1 text-right">Trades</th><th class="px-3 py-1 text-right">Win Rate</th></tr></thead><tbody>
            @for (r of compareRuns(); track r.runId) {
              <tr class="border-t border-gray-800"><td class="px-3 py-1 font-mono text-gray-400">{{ r.runId.slice(0, 8) }}</td><td class="px-3 py-1 text-right" [class.text-emerald-400]="r.netProfit>0" [class.text-red-400]="r.netProfit<0">{{ r.netProfit.toFixed(2) }}</td><td class="px-3 py-1 text-right">{{ (r.maxDrawdownPct * 100).toFixed(2) }}%</td><td class="px-3 py-1 text-right">{{ r.totalTrades }}</td><td class="px-3 py-1 text-right">{{ (r.winRatePct * 100).toFixed(1) }}%</td></tr>
            }
          </tbody></table></div>
        </div>
      }
      ...
      @else if (store.error()) { <div class="rounded-lg border border-red-800 bg-red-900/20 p-4 text-sm text-red-400">{{ store.error() }}</div> }
      @else if (store.runs().length === 0) {
        <div class="rounded-lg border border-gray-800 bg-gray-900/50 p-12 text-center"><p class="text-sm text-gray-500">No backtest runs yet.</p><a routerLink="/runs/new" class="mt-3 inline-block text-sm text-emerald-400 hover:underline">Start your first backtest</a></div>
      } @else {
        <div class="overflow-x-auto rounded-lg border border-gray-800"><table class="min-w-full text-sm"><thead class="bg-gray-900/50"><tr>
...
export class RunListComponent implements OnInit {
  ...
  isSelected(runId: string): boolean { return this.selectedRuns().includes(runId); }
  toggleSelect(event: Event, runId: string): void { event.stopPropagation(); this.selectedRuns.update(a => a.includes(runId) ? a.filter(x => x !== runId) : [...a, runId]); }
