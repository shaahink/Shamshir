            ...
            @for (s of strategies(); track s.id) {
              <label [attr.class]="stratClass(s.id)" (click)="toggleStrat(s.id)">
                ...
                <div>
                  ...
                  <div class="text-xs text-gray-500">Entry: {{ s.entryRule || '—' }} · Exit: {{ s.exitFormula || '—' }}</div>
                  <div class="text-xs text-gray-600">{{ s.id }}</div>
                </div>
              </label>
            }
          </div>
...
<label class="flex items-center gap-2 text-sm text-gray-500 cursor-pointer pt-1 border-t border-gray-700 mt-1">
  <input type="checkbox" [checked]="allRiskEnabled()" (change)="toggleAllRisk()" class="rounded" /> All risk controls
</label>
          </div>
        </div>
