...
> **STATUS — K1 DELIVERED 2026-06-20 (`iter/36-kernel-cutover`).**
> - **`BarEvaluator` (`Host/BarEvaluator.cs`)** — the new event producer. Per `BarClosed`: indicator window recompute → `IRegimeDetector` → `IStrategyBank.GetActive` → strategy eval → `EntryPlanner` → `ISignalGate`, emitting one `OrderProposed` per firing strategy (carrying `SlPips` + cross-rate-aware `PipValuePerLot` via the same `PipCalculator` path as `KernelOrderGate`/`MapOpenPositionsToProjected`) plus a per-strategy `StrategyVerdict` (signal/none + reason + indicators). Returns `BarEvaluation(Regime, Proposals, Verdicts)` and caches `Latest` for the driver's `evaluatorView` seam (wired in K3/K5). Order ids are a deterministic per-run counter (no `Guid.NewGuid`) so the proposal stream is replay-stable.
> - **External verdicts ported + made replay-safe.** `KernelOrderGate.ComputeVerdicts` (news/weekend/compliance/governor) is ported into the evaluator and evaluated at the **bar's sim-time**, not wall-clock — closing the latent date-dependency the K0 note flagged. The frozen verdict is carried on the event: `OrderProposed` gained a trailing `ExternalVerdicts External = default`, and `Kernel.DecideProposed` now threads `p.External` into `PreTradeGate.Evaluate` (was hard-coded `default`, silently dropping all external protection). So no protection is dropped, and a replay reproduces the same accept/reject bit-for-bit.
> - **`ExternalVerdicts` relocated Engine→Domain** (`Domain/Kernel/ExternalVerdicts.cs`) so the Domain-level event can carry it; `PreTradeGate` + `KernelOrderGate` updated to the moved type (behaviour-identical). The struct default = nothing blocks, so every existing `OrderProposed` construction (K0 tests) is unchanged.
> - **Tests (`KernelEvaluatorEquivalenceTests`, `[Speed=Fast]`):** (1) evaluator drives the kernel over the golden fixture → first proposal has `SlPips=50`,`PipValuePerLot=10`,Long; the gate sizes the first accept to **golden-snapshot.json's 0.20 lots/Long** with **exactly one `SubmitOrder`** (no double-submit); (2) a weekend verdict carried on a proposal makes the pure kernel reject with `WEEKEND_RESTRICTION` while the default-verdict control accepts (proves threading, not hollow); (3) the evaluator freezes `WeekendRestricted=true` from a Saturday bar's sim-time.
> - **No golden re-baseline.** The committed `golden-snapshot.json` is preserved (the evaluator reproduces its first order).
> - **Verified green:** K1 3/3; K0 GoldenReplay+Determinism+KernelAcceptance 9 pass/1 skip (the K3 full-run gate still `[Skip]`); Engine purity body-scan 4/4; Architecture 4/4; fast Simulation 21 pass/1 skip; Unit 209 pass/4 skip.
> - **Carried to K2/K3:** wire the `BarEvaluator` + the venue-feedback bridge into the actual kernel backtest loop (today the K1 test hand-drives the queue); fold `BarEvaluation.Verdicts`/`Regime` onto the BarClosed StepRecord via the driver's `evaluatorView` (K5).
...
> **STATUS — K2 DELIVERED 2026-06-20 (`iter/36-kernel-cutover`).**
> - **`KernelFeedback` (`Host/KernelFeedback.cs`)** — the pure venue→kernel bridge: `ExecutionEvent` → `OrderFilled`/`OrderPartiallyFilled`/`OrderCancelled`/`OrderRejected`; `AccountUpdate` → `EquityObserved`. Sim-time + price come off the venue event; no wall-clock. The kernel folds equity into the authoritative drawdown and runs the breach watchdog (`Kernel.DecideEquity`) — `PositionTracker`/`AccountProcessor` are no longer the authority on the kernel path.
> - **Id unification (the load-bearing K2 fix).** `OrderRequest` gained `Guid? ClientOrderId`; `EffectExecutor` submits under the kernel's order id (`= PositionId`); `BacktestReplayAdapter` + the test `FakeVenue` honor it. So the venue's open-trade book, execution events and closes all key off **one** id — the feedback bridge needs no venue-id↔kernel-id translation.
> - **Close-at-stop-price.** `CloseOpenPosition` gained `Price? ExitPrice`; `EngineReducer.HandleBarClosed` now sets the position `CloseReason` (so the venue close fill records "SL"/"TP", not "FORCE") and carries the stop/target price; `EffectExecutor` routes it to `ClosePositionAtAsync`. This is what makes a kernel SL/TP close fill at the stop price (matching the imperative `SimulateBarExitsAsync`) instead of the bar close.
> - **Tests (`KernelFeedbackTests`, `[Speed=Fast]`):** flat book → `EquityObserved(Equity==Balance)`, no false breach (C5 guard); a 6% drop enters protection **exactly once** (idempotent while protected); an SL close fill closes the Open position **via the reducer** → one `PublishTradeClosed` with `ExitReason=SL` + `ExitPrice` at the stop.
> - **Single watchdog on the kernel path:** breach/protection runs only in `Kernel.DecideEquity`; the imperative `AccountProcessor` survives only for the still-default old loop (deleted at K4).
> - **Carried to K4/K7:** Day/Week/Month roll **events** off the prop-firm reset time/zone (NEW-1) are not yet emitted by the loop — the golden fixture is a single UTC day so no roll is needed; multi-day roll detection is a pre-K4 follow-up. Threading venue commission/swap **through** the kernel `OrderFilled`→`PublishTradeClosed` (today the EffectExecutor recomputes gross; net==gross, which matches the zero-cost golden) is a K7 item for the real mark-to-market venue.
...
> **STATUS — K3 DELIVERED 2026-06-20 (`iter/36-kernel-cutover`).**
> - **`KernelBacktestLoop` (`Host/KernelBacktestLoop.cs`)** — the single `RunAsync` that drives `BarTape → BarEvaluator (K1) → Kernel → IEffectExecutor → venue + feedback bridge (K2)`, with `EngineState` threaded forward as the **single authority** and risk captured via `RiskSnapshots.Capture` onto one `StepRecord` journal per event. Per bar: advance venue → pump prior feedback → evaluate → enqueue proposals + `BarClosed` → pump (drain kernel queue + interleave venue executions to quiescence) → observe equity → pump. The pump is the in-order drain that makes the run deterministic.
> - **K3 gate GREEN** (`KernelBacktestLoopGoldenTests` + the now-**un-skipped** `KernelAcceptanceTests.KernelFullRun_MatchesGolden_TradesAndRisk`): the full golden run through the new loop reproduces `golden-snapshot.json`'s closed trade (Long 0.20 1.0970→1.0920 SL) **and** final risk (peak 10000, daily/max DD 1%, no protection) exactly, and is **bit-identical across two runs** (determinism). The first order sizes to 0.20 and the second to 0.19 off the realized equity drop — the full sizing trajectory the golden encodes.
> - **Equity model = realized (golden parity).** Golden was generated by the harness's realized-only equity model (floating excluded), so the loop feeds `EquityObserved` from `initial + closed net PnL` to reproduce it; the venue is the zero-cost `FakeVenue` (closes at the SL/TP price). The loop is **venue-agnostic** and already supports the production mark-to-market path (`realizedEquity: null` → drains the venue `AccountStream`), but that + `BacktestReplayAdapter` as the default is the **K4 flip**, which needs a reviewed golden re-baseline for the floating-DD difference (trade 2 stays open ⇒ mark-to-market DD > realized 1%).
> - **Old loop untouched.** `EngineRunner.RunBacktestLoopAsync` + the imperative twins remain present and default — the cutover flip + twin deletions are K4.
> - **Verified green:** K2+K3 6/6; the kernel category (K0+K1+K2+K3) 15/0; Engine purity 4/4; Architecture 4/4; fast Simulation 27/0; Unit 209/4-skip. No golden re-baseline.
