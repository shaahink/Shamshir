...
stage: **P4 — DONE on EURUSD.** `research parity --tape <id> --ctrader <id>` (exit 1 on FAIL). Gate proven falsifiable: it returned FAIL twice this session on this same code path before the fixes landed.
gate: build 0err/5warn · Unit 758/0/6 · Integration 121/0/0 · Sim-fast 144/0/0 · **live parity EURUSD PASS** (tape a89d37b5 / ctrader e497806d)
next: (1) XAUUSD + BTCUSD parity — one symbol hides scale-dependent bugs (INVESTIGATION-METHOD); each needs its venue spec captured (one cTrader run per symbol populates VenueSymbolSpecs). (2) **OWNER DECISION:** the tape now faithfully reproduces cTrader's M1 tick synthesis, artifacts included — limits/TPs fill BETTER than their level, which flatters results and X0 will inherit that bias. Closing it needs tick data on both legs. See PARITY-TRUTH-4 §5. (3) BTCUSD TRADES_LOST (F36) — trade capture on the cTrader leg is still lossy.
...
| **P4-xau** | **XAUUSD (14 trades, price 3369→4039): prices/lots/swap ALL GREEN** — the fill + swap models generalise across scale untouched. Commission FAILs on F47 (cTrader's artifact) | PARTIAL | (this commit) | tape `2c5a3f5e` / ctrader `a66fe9c4`: EntryPrice **0.0 ticks** · ExitPrice **100% within, 0.0 ticks** · Lots exact · Swap 1.37% · Commission 9.88% (F47) |
