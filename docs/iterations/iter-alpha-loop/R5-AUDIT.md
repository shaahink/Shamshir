...
| 3 | **New this audit — 9 `BacktestRuns` rows break PLAN §6's own "no lies" invariant** (`TotalTrades=0` while real `TradeResults` rows exist underneath them) | **Medium** | All 9 trace to two known live-verification sessions (3 from 2026-07-12 P2/P3 QA runs, 6 from the 2026-07-15 X0/X1 concurrency-proof runs) — test/proof artifacts, not census members, correctly excluded from scoring by the `status=completed` gate. Harmless to any result produced so far, but it means the plan's own headline health check currently returns a false positive. Fix: either finalize/clean these 9 rows, or scope the invariant query to `Status='completed'` (matching what D13's scoring gate already does) so a future session doesn't have to re-diagnose this from scratch. |
| 4 | **New this audit — both `baseline-sv1-prime` `Experiments` rows are stuck at `Status="Running"` forever**, including `075d5240` — the one actually adopted for R1'/R3/R4's entire ranking | **Medium** | Both rows were created by the pre-F59 buggy `ExperimentRunner` path (the same crash-and-leak bug F59 fixed) — they predate the fix and were never retroactively repaired. `ExperimentRuns` attachment and scoring are unaffected (confirmed: `075d5240` has all 252 runs correctly attached), so this is cosmetic today, but a future `research doctor`-style "stuck experiment" check would flag the iteration's own most important experiment row as broken. One-line `UPDATE Experiments SET Status='Completed' WHERE Id IN (...)` (or a proper `MarkCompleted` backfill) closes it. |
