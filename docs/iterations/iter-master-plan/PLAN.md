# MASTER PLAN — the multi-iteration program after iter-tape-trust
...
below is sized so one-or-two track phases ≈ one agent iteration. This document is the map; each iteration
...
(in flight on branch `iter/tape-trust` — do NOT modify that plan or duplicate its work) and the research
...
its T-phases merge). Never work in the main worktree while another agent is active there — `git worktree add`
a fresh one.

---

## §0 — Session warm-up (read this first, every fresh agent session)
...
1. `docs/iterations/iter-marketdata-tape/FULL-HANDOVER.md` — system tour (sections 1–8 accurate).
2. `docs/iterations/iter-marketdata-tape/HANDOVER-REVIEW.md` — verified reality + bug/gap vocabulary.
3. `docs/iterations/iter-tape-trust/PLAN.md` + its PROGRESS/VERIFICATION — what's already fixed (T0–T5).
4. `docs/QUANT-ROADMAP.md` — research methodology; Tracks F/G below implement its §5–§6.
5. This file.
...
- Gates: `dotnet test tests/TradingEngine.Tests.Unit` · Integration (91/91) · golden filter (see
  iter-tape-trust §gates) · `RequiresCTrader` E2E after any cBot change. Numbers → `docs/audit/PROGRESS.md`.
- One commit per phase; update the iteration's PROGRESS.md in the same commit.
- Owner reviews visually after UI phases — put a one-line "how to see it" note in PROGRESS.md per phase.
...
(T5) in flight. V2 bulk data download + V4/V5 real reconcile artifacts may still be pending — check
`iter-tape-trust` PROGRESS before assuming.

---

## §1 — Owner decision block (vote before the affected track starts; ✅ = default)

- **D-A1 Oracle cadence.** ✅ **A.** Weekly scheduled reconcile (one short cTrader run vs same-config tape run,
  artifact committed) + on-demand after any venue change. · B. Only on venue changes (cheaper, drift found late).
- **D-B1 Journal consolidation shape.** ✅ **A.** StepRecords (`JournalEntries` table) stay the ONE persisted
  ...
  "narrative" table written in parallel (risks divergence — rejected unless read-side proves too slow).
- **D-B2 Trade narrative persistence.** ✅ **A.** Add nullable columns to `Trades`
  (`EntryReason`, `EntryRegime`, `EntrySnapshotJson`, `ExitDetailJson`) stamped at open/close by the effect
  executor — cheap to query, survives journal pruning. · B. Join from journal at read time (no migration, but
  slow + breaks if journal thinned in sweep mode).
- **D-C1 Report layout.** ✅ **A.** Tabs (Overview · Trades · Journal · Bars/Why · Costs&Risk) replacing the
  single 12-section scroll. · B. Collapsible accordion sections (keeps one-page feel; still one giant DOM).
- **D-D1 Nav consolidation.** ✅ **A.** 6 top-level areas: **Live · Runs · Strategies · Risk (hub:
  profiles + FTMO rules + governor + packs) · Data · Settings**; Trades/Compare/cTrader-sessions move under
  Runs; API-docs link stays. · B. 8 areas (keep Trades top-level). Owner may re-letter freely — the hub idea
  is the point, not the exact grouping.
- **D-E1 DB reset semantics.** ✅ **A.** Three scoped, confirm-gated actions in Settings: *Clear runs*
  (runs/trades/equity/journal/orders/positions/events/sessions; config+marketdata survive), *Reseed config*
  (drop strategy/risk/pack/governor tables → reseed from JSON), *Wipe all* (recreate trading.db; marketdata.db
...
- **D-F1 Portfolio mechanism.** ✅ **A.** A named **Portfolio** config entity (strategy rows + per-row risk
  budget + activation rule) that New-Backtest can run as one unit; static allocation first, regime-based
  activation as the only "smart" rule in v1. · B. Free-form per-run row builder only (status quo — no reusable
  portfolio object).
- **D-G1 Symbol program scope.** ✅ **A.** Build the *scorecard + nomination report* (rank candidate symbols on
  measured cost/volatility/data quality); the OWNER picks the trading set (recommend 2–4 symbols); the system
...
## Track A — Venue-fidelity assurance program ("not experimenting on wrong calcs")
...
### A1 — Establish the oracle set + tolerance contract
- Curate 3–5 **oracle configs** (short cTrader runs with committed `shamshir-report.json`): at minimum
...
- Define the tolerance contract in `docs/audit/RECONCILE-FINDINGS.md`: per-trade net within X (start: 1× spread
  cost), aggregate NetProfit within Y, MaxDD within Z pts, TradeSet exact. Every number JUSTIFIED by a named
  residual gap (F3 trailing cadence, F5 commission timing, …) — no vibes tolerances.
- **Gate:** `scripts/reconcile-run.ps1` over all oracle configs → committed artifact per config, every row
...
- *Bid/ask semantics.* cTrader recorder bars are BID bars. After T3/F1, verify the convention is applied
  ...
  a unit test per case — this is where sign errors hide. ✅ Keep the spread constant per-symbol
...
### A2 — Continuous verification (the drift alarm)
- A small runner (script or `SweepRunnerService` sibling) that executes: 1 tape run per oracle config +
  reconcile vs the committed oracle report → PASS/FAIL per the tolerance contract. Wire as (a) an opt-in test
...
- Surface the latest verdict in the UI: a "Fidelity: verified <date> ✓ / drift ✗" chip on the Data page and on
  every tape run's report header.
- **Gate:** deleting one committed oracle trade (tamper test) flips the chip to drift.

**Guidance:** the committed-artifact test is the trick — it makes fidelity regression-testable WITHOUT cTrader
...
### A3 — Per-bar recorded spread (upgrade the cost model's weakest constant)
...
- Tape venue: use per-bar spread when present, else `TypicalSpread`. Reconcile A1 set again — expect the
  per-trade residual to tighten on volatile sessions.
- **Tricky:** the shard format is locked by `MarketDataShardIoTests.Parses_the_exact_cbot_recorder_line` — ADD
  a new locked test for the extended line, keep parsing the old line (backfill = null). cBot change ⇒ `.algo`
  rebuild + `RequiresCTrader` E2E.

---

## Track B — Journal & run-narrative backbone (the "actually useful journal")

**Today's reality (audited 2026-07-02) — four disjoint journal-ish stores:**
1. **`JournalEntries`** (SQLite) — the kernel StepRecord journal (every event: BarClosed, OrderProposed,
   fills, trails, equity, rolls) with Risk/Regime/Verdicts. The ONLY complete record. Feeds the report's
   "Journal", "Per-bar why" and "Bar Inspector" — three overlapping tables on one page.
2. **`state.RecentJournal`** — a 30-entry ring of coarse strings (SIGNAL/ORDER/EXEC/CLOSE/REJECTED/BREACH)
   built in `BacktestOrchestrator.TallyEvent` from progress callbacks; this—not the real journal—is what the
   Live Monitor shows. Different vocabulary, no SL/TP prices, no strategy verdicts, drops everything >30.
3. **`BacktestJournal`** (Web) — ephemeral `{eventType,message}` channel + `LogLines` string queue. Debug relic.
4. **`Events`/`Orders`/`Positions` tables + `VenueSessions`** — side ledgers, partially iter-33 era.
...
### B1 — Narrative projection (server-side)
- New read-side service `RunNarrativeService`: projects StepRecords → `NarrativeEvent { seq, simTime, severity
  ...
  - `OrderProposed+RecordDecisionEvent(Accepted)` → "trend-breakout LONG EURUSD @1.1347 — SL 1.1320 (27p), TP 1.1401 (2.0R), 0.74 lots (0.5% risk)"
  - `OrderFilled(close, CloseReason=SL)` → "Closed LONG EURUSD @1.1320 — stop-loss hit, −$412 net (−1.0R), held 6h"
  - `StopLossModifyRequested(TRAIL)` → "Trailed stop 1.1335 → 1.1349 (ATR×2.5)"
  - gate rejection → "Signal rejected: daily-DD budget could not absorb 0.5% risk (remaining 0.3%)"
- Endpoint `GET /api/runs/{id}/narrative?afterSeq=&kinds=&severity=` (cursor-paged — this IS the cache-plan-P6
  cursor read, do both at once). BarClosed/EquityObserved are EXCLUDED by default (opt-in kinds).
- **Gate:** for a seeded run, the narrative for one trade reads as a coherent 4–8 line story; cursor paging
...
- The needed fields live in `RawEvent`/`RawEffects` on fresh StepRecords but only as serialized
  ...
  `EventJson` (they're stable — golden-locked), with per-kind small parsers; tolerate missing fields (older
  runs) by degrading the headline, never throwing.
- ✅ Project at READ time (D-B1-A). If profiling shows re-projection cost matters for live polling, add a
  small per-run memo keyed by (runId, lastSeq) in `RunDataCache` — do not create a second persisted store.
...
### B2 — Live monitor feeds off the real journal
- Replace the `RecentJournal` ring as the monitor's source: monitor polls `/narrative?afterSeq=` every 1–2 s
  (or the SignalR envelope carries `latestSeq` and the client fetches the gap — ✅ simpler: poll; the envelope
...
- **Gate:** during a live tape run, the monitor journal shows the SAME lines (same seq) that the report shows
  after completion — one story, live or finished (the owner's core ask).
- **Tricky:** StepRecords reach SQLite via a buffered background sink — the cursor must read through
...
### B3 — Trade narrative ("why I entered / why I exited") [D-B2]
- Migration: nullable `EntryReason`, `EntryRegime`, `EntrySnapshotJson` (indicator values from the verdict +
  ...
  position state) in the effect executor — write-side only, NO kernel change (the data already flows through
  the executor).
...
- **Tricky:** partial closes produce multiple close fills per position — decide one `Trades` row semantics
...
### B4 — Report journal UX unification
- One `<app-journal>` component (kind chips + severity filter + search + cursor "load more") consuming the
  ...
  **Narrative** (default) and **Bar Inspector** (the per-bar table stays — it answers "why NO trade", which the
  narrative hides by design; fold "Per-bar why" verdicts INTO the Bar Inspector row expand).
- Journal NDJSON export stays (raw StepRecords — it's the audit artifact).
- **Gate:** report page renders ≤2 journal surfaces; WebSmoke green; owner visual pass.

---

## Track C — Charts that answer questions
...
(`candle-chart.component.ts:100-122`) — you genuinely cannot tell where entry/exit happened. "DD Timeline" in
...
### C1 — Trade chart: time-anchored entry/exit + level history
- Use lightweight-charts **series markers** (`createSeriesMarkers` in v5 — the repo is on the v5 API,
  `chart.addSeries(CandlestickSeries…)`): entry = arrow at the entry BAR (up/belowBar for long, down/aboveBar
  for short, label "Entry 1.1347"), exit = arrow at the exit bar labeled with reason + R ("SL −1.0R").
...
- Add ~20 bars of pre-entry context and shade the in-trade region (light direction-tinted band via
  `createPriceLine`-free approach: a translucent histogram/area series under the candles, or v5 panes; ✅
  simplest: keep candles + markers + step-lines, drop the shading if it fights the library).
- **Gate:** for a trailing-stop trade, the chart shows entry arrow → rising stop steps → exit arrow at the
...
- **Tricky:** marker times must be BAR times that exist in the series (lightweight-charts snaps/drops
  otherwise) — snap `openedAtUtc/closedAtUtc` to the chart TF's bar opens (the trade-chart endpoint already
...
### C2 — Drawdown views (daily bars + period timeline)
- **Server-side** `GET /api/runs/{id}/drawdown`: from EquitySnapshots compute per prop-firm day (22:00 UTC
  roll — reuse `ResetClock`/`ResetConfig`, do NOT group by calendar date) → `{ day, startEquity, minEquity,
...
- **UI on the report:** (a) DAILY DD bar chart — one red bar per day = maxDailyDdPct with a horizontal 5%
  limit line, breach days highlighted; (b) underwater (drawdown-vs-time) area chart under the equity curve —
...
- **Tricky:** intrabar equity (T3/F2 watermark) is what makes `minEquity` honest — if F2 emitted only
...
### C3 — Equity & balance, live and final, one semantics
- Unify: monitor and report equity charts both show Equity + Balance + underwater DD pane (monitor currently
  `showDrawdown=false` — turn it on once C2's series exists server-side; client-side derive is fine for live).
- Live monitor: append-only via `series.update()` instead of rebuilding `setData` on every frame (the
  current `equityData.update` + full `setData` path re-sorts 500 points per frame; fine but wasteful — low
  priority polish).
- Document (in code, one place) the equity semantics chain: venue AccountUpdate (mark-to-market, per bar +
  exits) → kernel EquityObserved → AccountSnapshot store (500 ms poll → SignalR) → EquityPersistenceHandler
  (5 s flush → EquitySnapshots). The monitor shows the polled value; the report shows persisted snapshots;
  after B2 both label the source.
- **Gate:** live equity at completion == report equity last point == run summary NetProfit + balance
  (already a report reconciliation badge — extend it to the monitor's final frame).

---

## Track D — App information architecture + the two key pages
...
### D1 — Nav consolidation + Risk hub [D-D1]
- 6 areas (see D-D1). **Risk hub** = one page with sub-tabs: Profiles · Prop-firm rules · Governor · Add-on
  packs, plus a "effective risk at a glance" summary strip (profile × ruleset × governor state) so the owner
  sees the whole money picture in one place. The 4 existing detail components are REUSED inside tabs (they're
  standalone components — mount, don't rewrite).
- Runs area gets sub-nav: All runs · Compare · Trades · cTrader sessions.
- **Gate:** every existing route still reachable (old paths redirect); WebSmoke green; nav ≤7 items.
- **Tricky:** deep links & routerLinkActive with child tabs — use child routes per tab, not component state,
  so refresh/back work.

### D2 — New-Backtest redesign
- Full-width two-pane layout: LEFT = what to run (strategy cards → row plan), RIGHT = sticky summary panel
...
- Inline data-coverage: when venue=tape, per selected (symbol, TF) show ✓/✗ coverage + m1-overlap (T1's
...
- **Tricky:** keep `StartRunRequest` unchanged (server contract stays; this is layout + affordances). The
  saved-setups strip (localStorage) stays but gets names ("Save as…" prompt) instead of timestamps.

### D3 — Run Monitor redesign
- Layout: top strip = progress + pass + ETA (keep); THEN a 2×2 grid: (1) equity+DD live chart (C3), (2) risk
  tiles (equity/balance/daily-DD with % of budget consumed as a bar, max-DD, governor, distance-to-limit —
  consolidate the current 8 tiles + 6 counters into these two groups), (3) live narrative journal (B2) with
  severity coloring, (4) open positions table (symbol, dir, lots, entry, current SL, floating PnL — from the
...
### D4 — Run Report information architecture [D-C1]
- Tabs: **Overview** (tiles + equity/DD + daily DD bars + daily PnL + run plan chips), **Trades** (table with
  DEFAULT 9 columns: Sym/Dir/Entry→Exit/Net/R/Pips/Exit reason/Strategy/Hold; column-chooser for the other 10;
  ...
  **Costs & Risk** (gross/comm/swap tiles, funnel + rejection histograms, MAE/MFE scatter, reconciliation
  badges, fidelity chip (A2), profiling), and header slimmed to: Duplicate · Export ▾ (journal/CSV/JSON/MD as
  a menu) · Monitor · All runs.
- **Gate:** every existing feature reachable in ≤2 clicks from the report; initial tab renders with ≤4 API
...
- **Tricky:** the 8 upfront fetches move into per-tab lazy loaders; keep the trades tab's expand cheap by
  fetching chart/narrative on expand only (already lazy for charts — keep).

### D5 — Settings becomes real + DB reset [Track E delivers the API]
- Settings page: system info from the API (`/api/system/info`: version/branch/build stamp — replace the
...
## Track E — Data & DB management

### E1 — Scoped database reset [D-E1]
- `POST /api/system/reset` with `{ scope: "runs" | "config" | "all" }`, admin-less (single-user app) but
  ...
  - **config:** delete StrategyConfigs/RiskProfiles/PropFirmRuleSets/GovernorOptions/AddOnPacks; re-run the
    seeders (they're idempotent startup services — expose their entry point).
  - **all:** close pooled connections (`SqliteConnection.ClearAllPools()` — REQUIRED on Windows or the file
...
- **Tricky:** the WAL files + connection pooling are the classic Windows failure ("file in use") — the
  ClearAllPools + rename dance is the reliable pattern; test on Windows, not just CI Linux.

### E2 — Runs housekeeping + marketdata hygiene
- Bulk ops on the runs list: multi-select → delete runs (same FK-safe cascade, scoped to ids); auto-prune
...
## Track F — Strategy portfolio & the smart picker [D-F1; methodology in QUANT-ROADMAP §5]

**Positioning (owner's stance honored):** the system is NOT tuning-centric. The portfolio work is about
*composition and gating*, driven by evidence from the sweep/exploration tooling — not parameter search.

### F1 — Portfolio entity + runner
- New `Portfolio` config (DB-seeded like packs): `{ id, name, rows: [{strategyId, symbol, tf, packId?,
  ...
  "Run portfolio <name>" (one click → the row plan), and the report gains a per-row contribution table
...
- Per-row `riskBudgetFraction` maps to the existing per-strategy risk override (`EffectiveConfigResolver`
  deep-merge) — composition without kernel change.
- **Gate:** a 3-row portfolio runs as one backtest; report shows per-row contribution; golden untouched.

### F2 — Correlation & contribution evidence (the picker's data)
- From any N finished runs (or one portfolio run), compute pairwise correlation of DAILY net PnL (22:00 roll)
  ...
  Cache nothing fancy — it's a few hundred days × a handful of streams.
- **Guidance:** correlation on daily PnL, not trade PnL (different trade counts break alignment); require ≥40
...
- **Gate:** two runs of the same strategy show corr ≈ 1.0 (sanity pin); trend-vs-mean-reversion shows visibly
  lower correlation on the owner's real data.

### F3 — Regime-gated activation (the only "smart" in v1)
- The regime filter already gates per-strategy entry. Elevate to the portfolio: activation rule per row
  (e.g. trend rows only in Trending, MR rows only in Ranging) — implemented as the EXISTING
...
- **Guidance for the picker recipe (owner-facing, documented on the portfolio page):**
  1. exploration-triage the 9 strategies per QUANT-ROADMAP §5 (kill the flat ones);
  2. pick 2–4 survivors from DIFFERENT families (trend / mean-reversion / session);
  3. check F2 correlation < ~0.4 pairwise on daily PnL;
  4. weight by inverse-DD (not by return) with per-row budgets summing ≤ total risk appetite;
  5. Monte Carlo the combined OOS trade stream → P(pass) (QUANT-ROADMAP §3.3) before adopting.
...
## Track G — Symbol program (research → features) [D-G1]

**The research answer first (owner's question "few symbols or many, can the system decide?"):**
- **Work on FEW symbols (2–4) deliberately chosen; let the system RANK, not choose.** More symbols only help
...
- **What makes a symbol GOOD for this system:** (1) low **cost-per-volatility** — spread (+commission in pips)
  ÷ ATR(14) on the trading TF; you're paying the spread out of each ATR-sized move (EURUSD H1: ~1 pip cost on
  ~15 pip ATR ≈ 7%; exotic pairs 3–5× worse); (2) session coverage matching the strategies' active hours;
...
- **Tricky:** ATR in PIPS needs the symbol's pip size (JPY!) — reuse `PipCalculator`/SymbolInfo, never 0.0001
  hardcode (B10's lesson). Correlation column is null until enough run data exists — show "—", not 0.

### G2 — Symbol-fit exploration (recipe, mostly tooling reuse)
- For a nominated symbol: run the exploration sweep (T5 + QUANT-ROADMAP Q1) of the surviving strategy family
  over the symbol's downloaded history; the scorecard page links "explore fit" → pre-filled sweep. The OUTPUT
  (post-cost expectancy per strategy family) is the final say on adoption — cost ranking nominates, backtests
  confirm.
- **Guidance:** don't cross-product everything; explore (nominated symbol × surviving strategies × owner TF)
  only. Symbols whose costPerAtrPct > ~15% on the target TF are rarely worth exploring at all on spread-paying
  strategies.

---

## §2 — Sequencing (iterations, dependencies, and what to run when)

```
Iteration 1  = B1 + B2            (narrative backbone + live monitor feed — unlocks C1/C2 journal reads, D3)
Iteration 2  = C1 + C2 + C3       (charts; needs B1 for stop-path + narrative categories)
Iteration 3  = D1 + D5 + E1       (nav hub + settings + DB reset — independent of B/C, can swap with 1–2)
...
Iteration 6  = A1 + A2            (oracle set + drift alarm; requires owner cTrader access; can run ANY time
                                   after tape-trust V4 — earlier is better, it's the trust anchor)
...
```
Rules: A-track can interleave anywhere (it's the safety net — prefer early). F/G MUST come after real data +
...
## §3 — What NOT to do (program-wide)

- Do NOT create a new persisted journal/table when a projection over StepRecords will do (D-B1).
- Do NOT let the monitor and report grow separate vocabularies again — one narrative builder, C# side.
- Do NOT group anything "daily" by calendar date — the prop-firm day rolls at 22:00 UTC (`ResetClock`).
