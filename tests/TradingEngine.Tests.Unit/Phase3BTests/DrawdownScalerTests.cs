...
public sealed class DrawdownScalerPhase3BTests
{
    ...
    {
        var factor = KernelSizing.ComputeScaleFactor(0.02m, 0.10m, 0.5, 0.5);
    ...
    {
        var factor = KernelSizing.ComputeScaleFactor(0.05m, 0.10m, 0.5, 0.5);
        factor.Should().Be(1.0);
    }
    ...
    {
        var factor = KernelSizing.ComputeScaleFactor(0.08m, 0.10m, 0.5, 0.5);
    ...
    {
        var factor = KernelSizing.ComputeScaleFactor(0.10m, 0.10m, 0.5, 0.5);
