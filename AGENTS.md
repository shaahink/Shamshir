...
**Created:** 2026-06-18
**Updated:** 2026-07-05 (P3.1 excursion recorder delivered)
...
- P3.1 (excursion recorder) delivered: tape-only, opt-in (`RecordExcursions` CustomParams flag, default off) per-trade MAE/MFE path capture. `TapeReplayAdapter` accumulates `(minutesSinceEntry, hiPips, loPips)` per fine bar for every open trade, serializes+attaches it to the CLOSE execution event, threaded end-to-end via the SAME 4-site `PositionLifecycle` pattern P0.1 used for `InitialStopLoss` (`ExecutionEvent`→`OrderFilled`→`PublishTradeClosed`→`TradeClosed`→`TradePersistenceHandler`'s existing trade-persistence channel, widened to also write a new `TradeExcursions` table via `IExcursionRepository`/`SqliteExcursionRepository`, migration `M36_TradeExcursions`) — kept off `TradeResult`/`TradeResultEntity` by design, a separate table not a new trade column.
...
- All gates green: Unit 419/0/6, Integration 100/0, fast Simulation 127/0 (~10s, cTrader-touching categories excluded — see below, byte-identical since RecordExcursions defaults off), Architecture 6/8 (2 pre-existing, undisturbed). The full cTrader-inclusive suite is deferred to end-of-P3 (owner override, mid-session); P2's own full-suite box is effectively already checked — that run caught and fixed one real regression (the CTraderBrokerAdapter `isLimit` revert, see PROGRESS.md's P2.7 write-up) before the owner's override landed. The deferred end-of-P3 run should cover P3's own changes.
